...
class User_swag_serializer(serializers.ModelSerializer):
    password = serializers.CharField(write_only=True)
    ...
    class Meta: 
        model = User
        fields = ('username', 'nickname', 'password', 'passwordconfirm', 'photo', 'is_active', 'is_admin', 'is_staff',)
...
class User_mypage_swag_serializer(serializers.ModelSerializer):
    photo = serializers.ImageField(use_url=True, required=False)
    class Meta:
        model = User
        fields = ('id', 'username', 'nickname', 'photo')
...
class User_password_swag_serializer(serializers.ModelSerializer):
    password = serializers.CharField(write_only=True)
    passwordconfirm = serializers.CharField(write_only=True)
    class Meta:
        model = User
